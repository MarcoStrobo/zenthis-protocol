...
async function main() {
  ...
  for (const id of ids) {
    ...
    const key = Object.entries({
      SEED: await vesting.SEED(), IDO: await vesting.IDO(),
      LIQUIDITY: await vesting.LIQUIDITY(), TEAM: await vesting.TEAM(),
      TREASURY: await vesting.TREASURY(), FOUNDER_OPS: await vesting.FOUNDER_OPS(),
    ...
    console.log(`   ${key.padEnd(14)} ${ethers.formatEther(s.totalAmount + s.tgeAmount).padStart(12)} ZENTHIS | cliff=${cliffMo}mo vest=${vestMo}mo`);
  }
  ...
  const tx = await htlc.newSwap(swapId, recipient, hash, now + 600, { value: ethers.parseEther("0.001") });
  await tx.wait();
